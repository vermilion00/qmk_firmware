...
void evaluate_joystick_axis(uint8_t index) {
    ...
    //TODO: Test this
    #ifdef USE_JOYSTICK
    uint16_t* const travel_diff_ptr = &key_config[index].release_value[active_profile];
    uint16_t* const value_ptr = &key_config[index].trigger_value[active_profile];
    #else
    uint16_t* const travel_diff_ptr = &key_config[index].joystick_travel;
    int16_t* const value_ptr = &key_config[index].joystick_value;
    #endif

    #ifndef INVERT_ADC
    if(scan_value > key_config[index].top_value - JS_TOP_DEADZONE) { *value_ptr = 0; }
    else if(scan_value < key_config[index].bottom_value + JS_BOTTOM_DEADZONE) { *value_ptr = 127; }
    else { *value_ptr = clamp_axis((((float)(key_config[index].top_value - scan_value - JS_TOP_DEADZONE)) / *travel_diff_ptr) * 127); }

    //TODO: If switching to ptr implementation, update this
    #else
    if(scan_value < key_config[index].top_value + JS_TOP_DEADZONE) { *value_ptr = 0; }
    else if(scan_value > key_config[index].bottom_value - JS_BOTTOM_DEADZONE) { *value_ptr = 127; }
    else { key_config[index].trigger_value[active_profile] = clamp_axis((((float)(scan_value - key_config[index].top_value - JS_TOP_DEADZONE)) / *travel_diff_ptr) * 127); }
    #endif
}
